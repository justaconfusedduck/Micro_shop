...
from flask import Flask, request, jsonify, make_response # Make sure 'request' is imported
...
if OTP_ENABLED and not all([MAILGUN_API_KEY, MAILGUN_DOMAIN, EMAIL_SENDER]):
    raise RuntimeError("Mailgun settings must be configured in .env when OTP is enabled.")
...
def admin_required(f):
    ...
    def decorated(*args, **kwargs):
        ...
        if request.method == 'OPTIONS':
            # --- UPDATED: Return a proper 200 OK for OPTIONS ---
...
def send_otp_email(recipient_email, otp):
    ...
    response = requests.post(
        ...
        data={"from": EMAIL_SENDER, "to": [recipient_email], "subject": "Your E-Commerce Verification Code", "text": f"Your one-time password is: {otp}\n\nThis code will expire in 5 minutes."})
...
def _create_and_set_tokens(username, user_role):
    ...
    # --- FIX: Added secure=True for SameSite=None cookie ---
    response.set_cookie(
        'refresh_token', 
        refresh_token, 
        httponly=True, 
...
def register_start():
    ...
    # --- MODIFIED: Split the validation check ---
    if not all([username, password, email]):
        return jsonify({"message": "Username, password, and email are required"}), 400
    
    # --- NEW: Conditionally check for CAPTCHA fields ---
    if CAPTCHA_ENABLED and not all([captcha_id, captcha_answer]):
    ...
    # --- NEW: Conditionally validate CAPTCHA answer ---
    if CAPTCHA_ENABLED:
...
@limiter.limit("5 per minute")
def login():
    data = request.get_json(); username = data.get('username'); password = data.get('password'); captcha_id = data.get('captcha_id'); captcha_answer = data.get('captcha_answer')
    ...
    if user and user.get('password') and isinstance(user.get('password'), str) and check_password_hash(user['password'], password):
        user_email = user.get("email")
        if OTP_ENABLED and user_email:
            otp = "".join(random.choices(string.digits, k=6))
            otp_expiry = datetime.now(timezone.utc) + PRE_AUTH_TOKEN_EXPIRES
            users_collection.update_one({"username": username}, {"$set": {"otp": otp, "otp_expiry": otp_expiry}})
            if not send_otp_email(user_email, otp): return jsonify({"message": "Failed to send verification email."}), 500
            pre_auth_token = jwt.encode({'sub': username, 'scope': 'pre-auth-otp', 'exp': otp_expiry}, SECRET_KEY, algorithm="HS256")
            return jsonify({'message': 'OTP sent to your email', 'pre_auth_token': pre_auth_token}), 206
...
@limiter.limit("5 per minute")
def verify_otp():
    data = request.get_json(); pre_auth_token = data.get('pre_auth_token'); otp_code = data.get('otp_code')
    if not pre_auth_token or not otp_code: return jsonify({'message': 'Token and OTP code are required!'}), 400
    try:
        payload = jwt.decode(pre_auth_token, SECRET_KEY, algorithms=["HS256"])
        if payload.get('scope') != 'pre-auth-otp': return jsonify({'message': 'Invalid token scope!'}), 401
        username = payload['sub']
        user = users_collection.find_one({"username": username})
        if not user or not user.get("otp") or not user.get("otp_expiry"): return jsonify({'message': 'OTP not found!'}), 400
        ...
        users_collection.update_one({"username": username}, {"$unset": {"otp": "", "otp_expiry": ""}})
        user_role = user.get("role", "buyer")
        return _create_and_set_tokens(username, user_role)
    except (jwt.ExpiredSignatureError, jwt.InvalidTokenError):
...
def refresh():
    ...
    if not token or not refresh_tokens_collection.find_one({'token': token}):
        return jsonify({'message': 'Refresh token is invalid or revoked!'}), 401
    try:
        data = jwt.decode(token, SECRET_KEY, algorithms=["HS256"])
        username = data['sub']
        user = users_collection.find_one({"username": username})
        ...
        user_role = user.get("role", "buyer")
        new_access_token = jwt.encode({'sub': username, 'role': user_role, 'type': 'access', 'exp': datetime.now(timezone.utc) + ACCESS_TOKEN_EXPIRES}, SECRET_KEY, algorithm="HS256")
...
# --- THIS IS THE FIX: Added 'OPTIONS' to the methods list ---
...
# --- THIS IS THE FIX: Added 'OPTIONS' to the methods list ---
@app.route("/admin/users/<string:username>/role", methods=['PUT', 'OPTIONS'])
@admin_required
...
if __name__ == '__main__':
    ...
    # --- NEW: Add log for CAPTCHA status ---
    print(f"CAPTCHA Verification is {'ENABLED' if CAPTCHA_ENABLED else 'DISABLED'}")
